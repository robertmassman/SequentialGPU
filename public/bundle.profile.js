...
class PipelineCacheManager {
    ...
    _hashString(str) {
        return GPUUtils$1.hashString(str);
    }
    ...
    */
    ...
    _formatShaderErrors(errors, code, label) {
        return GPUUtils$1.formatShaderErrors(errors, code, label);
    }
    ...
    _formatShaderMessage(message, codeLines = []) {
        return GPUUtils$1.formatShaderMessage(message, codeLines);
    }
    //////////////
...
class PipelineManager {
    ...
    createBindGroupLayout(filter, pass) {
        const layoutKey = GPUUtils$1.generateBindGroupLayoutKey(filter, pass);

        // Try cache first
        let layout = this.pipelineCacheManager.layoutCache.get(layoutKey)?.layout;

        if (!layout) {
            const entries = GPUUtils$1.createStandardLayoutEntries({ filter, pass });
            layout = this.device.createBindGroupLayout({ entries });
        ...
        layout.entries = GPUUtils$1.createStandardLayoutEntries({ filter, pass });
        return layout;
    }
    ...
    _generateDetailedPipelineKey(config) {
        return GPUUtils$1.generatePipelineKey(config);
    }
    ...
    createBindGroup(layout, filter, pass, bufferResource) {
        ...
        const entries = GPUUtils$1.createStandardBindGroupEntries({
            device: this.device,
        ...
        try {
            ...
            throw GPUUtils$1.handleError('PipelineManager', 'createBindGroup', error, {
...
class BindingManager {
    ...
    createBindings(resource = null) {
        ...
        this.bindGroupArray[groupIndex] = [{
            ...
            resource: GPUUtils$1.createStandardSampler(this.device)
        }];
    ...
    createDynamicBindGroupEntries(filter, pass) {
        return GPUUtils$1.createStandardLayoutEntries({ filter, pass });
    }
    ...
    createDynamicBindGroup(layout, filter, pass, bufferResource) {
        try {
            ...
            throw GPUUtils$1.handleError('BindingManager', 'createDynamicBindGroup', error, {
                filterLabel: filter.label,
                passLabel: pass.label
            });
        }
    ...
    _generateLayoutKey(filter, pass) {
        return GPUUtils$1.generateBindGroupLayoutKey(filter, pass);
    }
...
let RenderQueue$1 = class RenderQueue {
    constructor() {
...
class WebGpuRenderer {
   constructor(settings) {
      ...
      this.debugLogger = new DebugLogger$1(settings.debug);
      // Example debug log
      if (settings.debug) {
...
var performanceTracker$1 = /*#__PURE__*/Object.freeze({
    __proto__: null,
...
async function loadDebugModules() {
    ...
    try {
        const [debugModule, perfModule] = await Promise.all([
            Promise.resolve().then(function () { return debugLogger_enhanced; }),
            Promise.resolve().then(function () { return performanceTracker$1; })
        ]);
...
var debugLogger_enhanced = /*#__PURE__*/Object.freeze({
    __proto__: null,
